...
class MainApp(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        for i in range(3):
            if i < 2:
                buttons.append(tk.Button(self, text="Pull %d Pokemon" % ((i+1)*3),
                                         command=lambda i=i: self.choose((i+1)*300, (i+1)*3)))
            else:
                buttons.append(tk.Button(self, text="Pull %d Pokemon" % ((i+1)*4),
                                         command=lambda i=i: self.choose((i+1)*400, (i+1)*4)))
            buttons[i].grid(row=0, column=i)

        self.label = tk.Label(self, text="Get %s within %d more 12-pulls!" % (self.names[self.bannerNum], getPlayer(
            self.currentPlayer.get()).numPullsUntil[self.bannerNum]), font=('Verdana', 16))
        ...
        for i in range(numpokes[self.bannerNum]):
            ...
            text = "%15s | %10s | %.3f %%" % (
                pkmn_list[self.bannerNum][i].name, pkmn_list[self.bannerNum][i].rarity, prob)
        ...
        self.dropdown = tk.OptionMenu(self, self.currentPlayer, *
                                      playerNames, command=self.updatePlayerInfo)
        self.dropdown.grid(row=18, column=1)
        self.jewelAmount = tk.Label(self, text=str(
            getPlayer(self.currentPlayer.get()).jewels) + " Jewels", font=('Monospace', 10))
    ...
    def viewInventory(self):
        ...
        sortA = tk.Button(self.inventoryMenu, text="Sort Alphabetically",
                          command=lambda: self.sortAlpha(getPlayer(self.currentPlayer.get())))
        sortA.grid(row=0, column=0)
        sortB = tk.Button(self.inventoryMenu, text="Sort by Rarity",
                          command=lambda: self.sortRare(getPlayer(self.currentPlayer.get())))
        ...
        for i in range(len(getPlayer(self.currentPlayer.get()).team)):
            self.inventoryMenu.labels.append(tk.Label(self.inventoryMenu, text="%15s   %10s" % (getPlayer(
                self.currentPlayer.get()).team[i].name, getPlayer(self.currentPlayer.get()).team[i].rarity), font=('Monospace', 10)))
    ...
    def updateInventory(self):
        try:
            ...
            for i in range(len(getPlayer(self.currentPlayer.get()).team)):
                try:
                    self.inventoryMenu.labels[i].config(text="%15s   %10s" % (getPlayer(
                        self.currentPlayer.get()).team[i].name, getPlayer(self.currentPlayer.get()).team[i].rarity))
                    self.inventoryMenu.labels[i].grid(row=i % j+2, column=int(i/j), sticky="e")
                except IndexError:
                    self.inventoryMenu.labels.append(tk.Label(self.inventoryMenu, text="%15s   %10s" % (getPlayer(
                        self.currentPlayer.get()).team[i].name, getPlayer(self.currentPlayer.get()).team[i].rarity), font=('Monospace', 10)))
    ...
    def choose(self, jewels, numToChoose):
        ...
        if getPlayer(self.currentPlayer.get()).jewels - jewels >= 0:
            ...
            self.jewelAmount.config(
                text=str(getPlayer(self.currentPlayer.get()).jewels) + " Jewels")
                    ...
                    if i == self.bannerNum:
                        for pokemon in pull:
                            if pokemon.name == self.names[i]:
                                getPlayer(self.currentPlayer.get()
                                          ).numPullsUntil[self.bannerNum] = 3
                                noMercy = False
                                break
                            else:
                                noMercy = True
                        ...
                        self.label.config(text="Get %s within %d more 12-pulls!" %
                                          (self.names[i], getPlayer(self.currentPlayer.get()).numPullsUntil[self.bannerNum]))
    ...
    def nextBanner(self, banNum):
        ...
        self.label.config(text="Get %s within %d more 12-pulls!" %
                          (self.names[self.bannerNum], getPlayer(self.currentPlayer.get()).numPullsUntil[self.bannerNum]))

    def updatePlayerInfo(self, info):
        if self.bannerNum < 14:
            self.label.config(text="Get %s within %d more 12-pulls!" %
                              (self.names[self.bannerNum], getPlayer(info).numPullsUntil[self.bannerNum]))
